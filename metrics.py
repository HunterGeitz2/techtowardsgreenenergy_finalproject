...
def calculate_data_availability(logs: list[dict]) -> float:
    if not logs:
        return 0.0

    available = sum(1 for row in logs if row["packet_received"])
    return 100.0 * available / len(logs)


def calculate_incoming_data_integrity(logs: list[dict]) -> float:
    if not logs:
        return 0.0

    received_logs = [row for row in logs if row["packet_received"]]
    if not received_logs:
        return 0.0

    untampered = sum(1 for row in received_logs if not row["packet_tampered"])
    return 100.0 * untampered / len(received_logs)


def calculate_trusted_control_data(logs: list[dict]) -> float:
    if not logs:
        return 0.0

    trusted = sum(1 for row in logs if row["controller_data_trusted"])
    return 100.0 * trusted / len(logs)
...
def summarize_metrics(logs: list[dict], scenario_name: str) -> dict:
    return {
        ...
        "data_availability_percent": round(calculate_data_availability(logs), 2),
        "incoming_data_integrity_percent": round(calculate_incoming_data_integrity(logs), 2),
        "trusted_control_data_percent": round(calculate_trusted_control_data(logs), 2),
